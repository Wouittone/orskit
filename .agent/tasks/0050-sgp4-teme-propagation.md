...
- Keep the standard `TryFrom<&TwoLineElement> for Orbit<Sgp4Elements>`
  conversion at the I/O boundary; the `dynamics` crate's gated `sgp4` module
  has no `TwoLineElement` dependency.
