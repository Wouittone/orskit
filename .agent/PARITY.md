...
| I/O | CCSDS orbit, attitude, tracking, and navigation messages | Partial | Conformance corpus; lossless semantic round trips | CCSDS 502.0-B-3 OEM KVN blocking/Tokio event ingestion and ordered Rayon collection use finite resource budgets and enforce strictly increasing epochs per segment before emission/assembly; explicit enrichment supplies physical state properties absent from OEM; XML, covariance, semantic provenance preservation, other message families, conformance corpus, and writing remain pending |
