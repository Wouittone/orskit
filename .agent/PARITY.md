...
| Propagation | Numerical integration and dense ephemerides | Partial | Integrator order/error tests; independent scenarios | The feature-gated `dynamics-numerical` crate implements typed Cartesian `BogackiShampine32` propagation over caller-selected evaluable dynamics. It uses explicit position/velocity absolute tolerances, relative scaling, step/rejection limits, exact target epochs, forward/backward stepping, and immutable `CartesianEphemeris` output over retained cubic-Hermite accepted steps. Manufactured polynomial and observed third-order tests, typed failure cases, analytical two-body comparison, the recorded Orekit endpoint, dense endpoint/interior cases, and reverse coverage pass. Coupled mass/attitude/variational layouts, higher-order methods, operational scenarios, and a performance claim remain pending |
...
| Propagation | Events and root localization | Partial | Direction/grazing/simultaneous-event tests | `EventDetector` supplies a finite dimensionless switching value and propagation-order rising/falling/any selection. Accepted steps are capped by a typed maximum-check interval; sign-change or exact-endpoint roots are localized by bounded bisection over dense states. Occurrences dispatch by propagation time, with roots inside the time tolerance dispatched in detector registration order; every simultaneous handler runs before a requested stop truncates the ephemeris. Forward/reverse direction and ordering, shared-boundary de-duplication, simultaneous stop, bounded failures, and the explicit unbracketed-grazing limitation pass. State reset, discontinuities, detector-specific check intervals, and unbracketed grazing localization remain pending |
