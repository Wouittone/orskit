...
| Propagation | Dynamics and force-model composition | Designed | Multi-model topology; explicit participants/data needs; coupled-state evaluation scenarios | Description-only `SystemDynamics` splits open conservative/non-conservative force contracts and declares spacecraft-state dependencies; validated two-/three-body topology and plug-in ordering; evaluation/data/state contracts pending |
