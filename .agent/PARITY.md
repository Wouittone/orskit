...
| Geometry | Frames, transforms, Earth orientation | Partial | Transform composition/inverse; independent frame vectors | Affirmative motion composes with caller-owned `FrameCatalog`; namespace-qualified `FrameId` prevents cross-catalog collisions, definitions reject conflicts/foreign parents and form registered acyclic chains. `FrameKinematics` and the open `KinematicFrameTransformProvider` make the epoch, source/target identities, and finite kinematics explicit. `FrameReferenceDataSupplier` exposes immutable source/product/revision/checksum provenance, while `ReferenceDataKinematicFrameTransform` delegates distinct-frame requests and verifies the result frame; the only data-free provider permits identity only. Earth-orientation equations, a concrete supplier, and independent transform vectors remain pending |
