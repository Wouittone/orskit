...
| Orbits | Epoch/frame-qualified Cartesian states | Partial | Invariant-bearing API; units/frame/time tests | `CartesianState` stays gravity-independent; element states share an `Arc<dyn CentralGravity>` and Cartesian/element conversion validates allocation identity plus frame origin; `Orbit` qualifies the closed representation set with its epoch; bound conversion has typed gravity/frame/degeneracy/conic errors |
...
| Propagation | Two-body/Keplerian propagation | Partial | Analytic orbit scenarios; conservation/error budget | Object-safe `Propagator<Problem>` separates method from topology; `EllipticKeplerPropagator` targets strict `TwoBodyDynamics`, shares central-gravity identity, and preserves the orbital enum variant. Existing representation/reference/benchmark evidence remains; numerical/stochastic methods, other conics, events, and ephemerides remain pending |
