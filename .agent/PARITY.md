...
| Propagation | Numerical integration and dense ephemerides | Partial | Integrator order/error tests; independent scenarios | The feature-gated `dynamics-numerical` crate implements typed Cartesian `BogackiShampine32` propagation over caller-selected evaluable dynamics. It uses explicit position/velocity absolute tolerances, relative scaling, step/rejection limits, exact target epochs, forward/backward stepping, and private cubic-Hermite accepted-step output. Manufactured polynomial and observed third-order tests, typed failure cases, analytical two-body comparison, and the recorded Orekit endpoint pass. Public dense ephemerides, events, coupled mass/attitude/variational layouts, higher-order methods, operational scenarios, and a performance claim remain pending |
