...
| Orbits | Epoch/frame-qualified Cartesian states | Partial | Invariant-bearing API; units/frame/time tests | `orskit-core` exposes open `SpacecraftState` and `Orbit<S>` contracts; `orskit-orbits-cartesian` supplies gravity-independent `CartesianState` and element states sharing an `Arc<dyn CentralGravity>`; conversion has typed gravity/frame/degeneracy/conic errors |
...
| Propagation | Two-body/Keplerian propagation | Partial | Analytic orbit scenarios; conservation/error budget | Feature-gated `orskit-dynamics-two-body` supplies strict `TwoBodyDynamics` and `EllipticKeplerPropagator`; it targets `Orbit<S>` for Cartesian, Keplerian, equinoctial, or its local convenience enum, uses exact duration nanoseconds, and advances equinoctial longitude without a singular Keplerian intermediate |
